...
async function loadQuestions() {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            let response = await fetch(`${APPS_SCRIPT_URL}?action=getQuestions&t=${Date.now()}`, {
                method: 'GET',
                headers: {
            ...
            if (!response.ok) {
                ...
                response = await fetch(`${APPS_SCRIPT_URL}?t=${Date.now()}`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                        'Cache-Control': 'no-cache'
...
window.testLoadQuestions = async function() {
    ...
    try {
        ...
        return { success: true, message: 'Questions loaded successfully' };
    } catch (error) {
