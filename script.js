...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwC3a-03md3BBjYAFkSaNr2Bc9HmIghf5mF9YPK-gOqLigkxjXlLyBOzBpauspBvW7CwA/exec';
...
function setupEventListeners() {
    ...
    document.getElementById('newStudentBtn').addEventListener('click', newStudent);
...
function startQuiz() {
    ...
    document.getElementById('studentGradeDisplay').textContent = studentData.grade;
...
function showQuestion() {
    ...
    // Show answer required warning
    document.getElementById('answerRequiredWarning').classList.remove('hidden');
 ...
 */
function showAnswerFeedback(selectedLetter, correctLetter) {
    const options = document.querySelectorAll('.option');
    ...
    options.forEach(option => {
        ...
        } else if (optionLetter === selectedLetter && selectedLetter !== correctLetter) {
...
function updateNavigationButtons() {
    ...
    // Only show next/finish button if current question is answered
    if (hasAnsweredCurrentQuestion) {
        ...
        // Hide both buttons if question not answered
        nextBtn.classList.add('hidden');
        finishBtn.classList.add('hidden');
    }
}
...
function nextQuestion() {
    ...
    if (!hasAnsweredCurrentQuestion) {
        showError('يجب عليك اختيار إجابة قبل الانتقال للسؤال التالي');
        return;
    }
...
async function finishQuiz() {
    // Check if current question is answered
    if (!hasAnsweredCurrentQuestion) {
        showError('يجب عليك اختيار إجابة قبل إنهاء الامتحان');
        return;
    }
    ...
    try {
        ...
        if (!navigator.onLine) {
            console.log('Device is offline, saving results locally only');
            showResults();
            updateLocalExamHistory();
            showWarningMessage('أنت غير متصل بالإنترنت. تم حفظ النتائج محلياً فقط.');
            return;
        }
        
        // Try to save results to Google Sheets via Apps Script
        let saveSuccess = false;
        let saveError = null;
        
        try {
            console.log('Trying to save results to Google Sheets...');
            ...
            try {
                ...
                try {
                    ...
                    throw new Error('فشل في الاتصال بالخادم. سيتم حفظ النتائج محلياً فقط.');
                }
            }
            ...
            if (response.ok) {
                ...
                if (result.success) {
                    saveSuccess = true;
                    console.log('Results saved successfully to Google Sheets');
                } else {
                    saveError = result.error || 'فشل في حفظ النتائج';
                    console.log('Google Sheets save failed:', saveError);
                }
            } else {
                saveError = `HTTP error! status: ${response.status}`;
                console.log('HTTP error:', saveError);
            }
            
        } catch (networkError) {
            saveError = networkError.message;
            console.log('Network error:', saveError);
        }
        
        // Always show results and update local history
        showResults();
        updateLocalExamHistory();
        ...
        if (saveSuccess) {
            ...
            setTimeout(() => {
                showSuccessMessage('تم حفظ النتائج بنجاح في قاعدة البيانات');
            }, 1000);
        } else {
            // Show warning that results are saved locally only
            setTimeout(() => {
                showWarningMessage('تم حفظ النتائج محلياً. قد لا تكون متاحة على الأجهزة الأخرى.');
            }, 1000);
        }
...
function handleOffline() {
    ...
    showWarningMessage('انقطع الاتصال بالإنترنت. سيتم حفظ النتائج محلياً.');
}
